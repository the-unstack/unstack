#!/bin/bash
# initialize, down (descending), then up in two passes: infra first, rest after. Exit 1 if any service failed.
...
rc=0
"$DIR/_for_each_service.sh" -r down --remove-orphans || rc=1 # a failed down must not skip up
